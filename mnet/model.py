...
def conv1x1(in_planes, out_planes, stride=1, bias=False, groups=1):
    "1x1 convolution"
    return nn.Conv2d(in_planes, out_planes, kernel_size=1, stride=stride, padding=0, bias=bias, groups=groups)

def batchnorm(in_planes):
    "batch norm 2d"
    return nn.BatchNorm2d(in_planes, affine=True, eps=1e-5, momentum=0.1)

def convbnrelu(in_planes, out_planes, kernel_size, stride=1, groups=1, activation=True):
    "conv-batchnorm-relu"
    if activation:
...
class CRPBlock(nn.Module):
    """CRP Block from https://arxiv.org/abs/1809.04766"""
...
class InvertedResidualBlock(nn.Module):
    ...
    def __init__(self, in_planes, out_planes, expansion_factor, stride=1):
        ...
        self.output = nn.Sequential(convbnrelu(in_planes, intermed_planes, 1),
                                    ...
                                    convbnrelu(intermed_planes, out_planes, 1, activation=False))
...
class Net(nn.Module):
    ...
    in_planes = 32 # number of input channels (output of the first layer)
    ...
    def __init__(self, num_classes, num_tasks=2):
        ...
        layer = 2
        for expansion_rate, output_channels, number_of_repeats, stride in (self.mobilenet_config):
            layers = []
            for idx in range(number_of_repeats):
                layers.append(InvertedResidualBlock(self.in_planes, output_channels, expansion_factor=expansion_rate, stride=stride if idx == 0 else 1))
                self.in_planes = output_channels
            setattr(self, 'layer{}'.format(layer), nn.Sequential(*layers))
            layer += 1  
        ...
        self.crp4 = self._crp(256, 256, 4, groups=False)
        self.crp3 = self._crp(256, 256, 4, groups=False)
        self.crp2 = self._crp(256, 256, 4, groups=False)
        self.crp1 = self._crp(256, 256, 4, groups=True)
    ...
    def _crp(self, in_planes, out_planes, stages, groups=False):
        layers = [CRPBlock(in_planes, out_planes,stages, groups=groups)]
        return nn.Sequential(*layers)
    
def net(num_classes, num_tasks=2):
